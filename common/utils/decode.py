...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    """
    将神经网络输出的特征图（heatmap、宽高、回归信息等）转换为检测框（bounding boxes），并返回检测结果
    """
    # 获取输入尺寸
    batch, cat, height, width = heat.size()
    # heat = torch.sigmoid(heat)
    ...
    # 2.1 之后经过_topk：
    # _topk 函数从经过 NMS 处理的 heatmap 中选取前 K 个得分最高的点，并返回它们的分数、索引、类别、y 坐标和 x 坐标
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    # 如果 reg 不为 None，说明使用了回归偏移信息，对中心点进行微调，提高定位精度
    if reg is not None:
        reg = _transpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, K, 2)
        xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
        ...
        xs = xs.view(batch, K, 1) + 0.5
        ys = ys.view(batch, K, 1) + 0.5
    # 根据 cat_spec_wh 选择是否为每个类别使用独立的宽高预测。
    # 如果使用了类别特定的宽高预测，代码会根据类别索引提取对应的宽高信息
    wh = _transpose_and_gather_feat(wh, inds)
    if cat_spec_wh:
    ...
    clses = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
    # 将中心点坐标和宽高信息结合，计算出每个检测框的左上角和右下角坐标
    bboxes = torch.cat([xs - wh[..., 0:1] / 2,
                        ys - wh[..., 1:2] / 2,
                        xs + wh[..., 0:1] / 2,
                        ys + wh[..., 1:2] / 2], dim=2)
    # 将检测框、分数和类别信息拼接在一起，得到最终的检测结果
