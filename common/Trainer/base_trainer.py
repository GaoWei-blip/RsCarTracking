...
from common.utils.data_parallel import DataParallel
from common.utils.utils import AverageMeter
...
class BaseTrainer(object):
    ...
    def run_epoch(self, phase, epoch, data_loader):
        ...
        for iter_id, (im_id, batch) in enumerate(data_loader):
            ...
            """
            hm: (B, C, H_out, W_out) 每类的得分
            wh: (B, 2, H_out, W_out) 边界框的宽高
            reg: (B, 2, H_out, W_out) 边界框中心坐标的偏移量
            """
            print('phase=%s, epoch=%5d, iters=%d/%d,time=%0.4f, loss=%0.4f, hm_loss=%0.4f, wh_loss=%0.4f, off_loss=%0.4f' \
                  % (phase, epoch,iter_id+1,num_iters, time.time() - end,
                     ...
                     loss_stats['off_loss'].mean().cpu().detach().numpy()), flush=True)
